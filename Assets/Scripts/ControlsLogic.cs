...
public class ControlsLogic : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!GameManager.levelFailed && GameManager.levelStarted)
        {
            ...
            {
                rb.linearVelocity -= new Vector3(0, 0.05f, 0);  // acceleration speed
            }
            else if (Mathf.Abs(rb.linearVelocity.y - maxFallSpeed) < 0.02f)
            {
                rb.linearVelocity = new Vector3(rb.linearVelocity.x, maxFallSpeed, rb.linearVelocity.z);
            }
            else if (rb.linearVelocity.y < maxFallSpeed)
            {
                rb.linearVelocity += new Vector3(0, 0.26f, 0);  // decelleration speed
            }
            else
                rb.linearVelocity = new Vector3(rb.linearVelocity.x, maxFallSpeed, rb.linearVelocity.z);
        }

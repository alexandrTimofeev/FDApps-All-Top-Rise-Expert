...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private ParticleSystem skinChangeParticles;

    [SerializeField] private ChangePlaneVisuale _ChangesVisual;
    ...
    public void Move(Vector3 _velocity)
    {
        if (GameManager.levelStarted && !GameManager.levelFailed)
        {
            if (ControlsLogic.touchedDown)
                velocity = new Vector3(_velocity.x, myRigidBody.linearVelocity.y, velocity.z);
            else
            {
                if (!leveledOut)
                {
    ...
    IEnumerator LevelOut(Vector3 _velocity)
    {
        ...
        for (float i = 0; i <= endCount; i++)
        {
            ...
            velocity = new Vector3(pullBack, myRigidBody.linearVelocity.y, velocity.z);
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            // Сброс визуала при смерти
            if (_ChangesVisual != null)
            {
                _ChangesVisual.ResetToMeshes();
            }

            transform.GetComponent<MeshRenderer>().enabled = false;
            if (myRigidBody.linearVelocity.y > 0.475f)
                explosionSystem.startSpeed = 7 + ((Mathf.Abs(myRigidBody.linearVelocity.y) - 0.475f) / 2.425f * 7); // 7 to 13
            else
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.GetComponent<ChangeTokenVisual>())
        {
            _ChangesVisual.TogglePlaneVisual();
        }
    }
